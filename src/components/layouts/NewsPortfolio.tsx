...
import { portfolioNews } from "../../store/features/portfolioSlice";
...
function NewsPortfolio({portfolio_id}:any){
    ...
    const getPortfolioNews=()=>{
        setloading(true)
        dispatch(portfolioNews({portfolio_id}) as any).then(unwrapResult).then((res:any)=>{
            setloading(false);
            if(res && res.code==200){
                setData(res.data);
            }
